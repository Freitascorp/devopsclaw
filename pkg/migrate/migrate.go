...
func Run(opts Options) (*Result, error) {
	...
	picoClawHome, err := resolveDevOpsClawHome(opts.DevOpsClawHome)
	if err != nil {
		return nil, err
	}
	...
	fmt.Println("Migrating from OpenClaw to DevOpsClaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Description: "convert OpenClaw config to DevOpsClaw format",
			})
...
func executeConfigMigration(srcConfigPath, dstConfigPath, picoClawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing DevOpsClaw config: %w", err)
		}
...
func resolveDevOpsClawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("DEVOPSCLAW_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".devopsclaw"), nil
}
