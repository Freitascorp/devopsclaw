...
import (
	...
	"github.com/freitascorp/devopsclaw/pkg/browser"
	...
	"github.com/freitascorp/devopsclaw/pkg/constants"
	"github.com/freitascorp/devopsclaw/pkg/logger"
	"github.com/freitascorp/devopsclaw/pkg/providers"
	"github.com/freitascorp/devopsclaw/pkg/rbac"
	"github.com/freitascorp/devopsclaw/pkg/routing"
...
type processOptions struct {
	...
	SenderID        string // Sender identity for RBAC checks
...
func registerSharedTools(
	...
	for _, agentID := range registry.ListAgentIDs() {
		...
		// Browser automation tool — register when enabled
		if cfg.Browser.Enabled {
			browserTool := browser.NewBrowserTool(&browser.ManagerConfig{
	...
	for iteration < agent.MaxIterations {
		...
		for _, tc := range normalizedToolCalls {
			...
			if al.toolGuard != nil {
				...
				if opts.Channel != "" && opts.SenderID != "" {
					if resolved := al.toolGuard.ResolveUser(opts.Channel, opts.SenderID); resolved != "" {
						userID = resolved
					}
				}
				if err := al.toolGuard.CheckToolAccess(ctx, userID, tc.Name); err != nil {
					logger.WarnCF("agent", "RBAC denied tool access",
						map[string]any{
							"user":  string(userID),
							"tool":  tc.Name,
							"error": err.Error(),
