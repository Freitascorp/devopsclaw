...
import (
	...
	"github.com/freitascorp/devopsclaw/pkg/auth"
...
func authHelp() {
	...
	fmt.Println("  devopsclaw auth login --provider openai")
	fmt.Println("  devopsclaw auth login --provider openai --device-code")
	fmt.Println("  devopsclaw auth login --provider anthropic")
	fmt.Println("  devopsclaw auth login --provider google-antigravity")
	fmt.Println("  devopsclaw auth models")
	fmt.Println("  devopsclaw auth logout --provider openai")
	fmt.Println("  devopsclaw auth status")
}
...
func authLoginGoogleAntigravity() {
	...
	fmt.Println("Try it: devopsclaw agent -m \"Hello world\"")
}
...
func authStatusCmd() {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: devopsclaw auth login --provider <name>")
		return
	}
...
func authModelsCmd() {
	...
	if err != nil || cred == nil {
		...
		fmt.Println("Run: devopsclaw auth login --provider google-antigravity")
		return
	}
