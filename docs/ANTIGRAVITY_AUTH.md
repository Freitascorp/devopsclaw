...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in DevOpsClaw.
...
10. [Creating a New Provider in DevOpsClaw](#creating-a-new-provider-in-devopsclaw)
...
const antigravityPlugin = {
  ...
  register(api: DevOpsClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<DevOpsClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- DevOpsClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.devopsclaw/auth.json`:
...
## Creating a New Provider in DevOpsClaw

DevOpsClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/devopsclaw/cmd_auth.go`:
...
devopsclaw auth login --provider your-provider

# List models (for Antigravity)
devopsclaw auth models

# Start the gateway
devopsclaw gateway

# Run an agent with a specific model
devopsclaw agent -m "Hello" --model your-model
```
...
export DEVOPSCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export DEVOPSCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `devopsclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.devopsclaw/auth.json`
- Re-run `devopsclaw auth login --provider antigravity`
