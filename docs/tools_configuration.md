...
DevOpsClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, DevOpsClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `DEVOPSCLAW_TOOLS_<SECTION>_<KEY>`:

For example:
- `DEVOPSCLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `DEVOPSCLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `DEVOPSCLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
